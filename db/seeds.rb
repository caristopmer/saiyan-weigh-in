
# User.create(username: "izzlebeast", email: "chris@chris.com", password: "password", password_confirmation: "password", height: 74, height_unit: "inches")
# User.create(username: "AmandaH", email: "amanda@amanda.com", password: "password", password_confirmation: "password", height: 67, height_unit: "inches")

# Entry.create(user_id: 1, weight: 220, weight_unit: "lbs.", entry_date: Date.today - 5)
# Entry.create(user_id: 1, weight: 210, weight_unit: "lbs.", entry_date: Date.today - 3)
# Entry.create(user_id: 1, weight: 200, weight_unit: "lbs.", entry_date: Date.today)

team = Team.create(name: "The Maddi Monsters", captain: User.find_by(username: "izzlebeast"))

team.members << User.find_by(username: "izzlebeast")
team.members << User.find_by(username: "AmandaH")
team.save